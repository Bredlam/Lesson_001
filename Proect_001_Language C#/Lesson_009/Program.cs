...
// Пример кода: циклический ввод переменной типа double
double varA;

while (true)
{
    Console.Write("Введите А: ");
    if (double.TryParse(Console.ReadLine(), out varA))  // в этой строке при вводе данных проверяется тип значения данных и если
                                                        // он не подходит под назначенный (в данном случае это double) то условие
                                                        // оператора if выполняется и происходит прерывание - break
    {
        break;
    }
